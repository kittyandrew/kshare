...
- **The code is the spec.** Every behaviour is documented at the
  ...
  Cross-cutting decisions are indexed across the next two bullets.
- `.claude/rules/` is the operating manual; auto-loaded every
  session. `001-architecture.md` is the topology + scope;
  `003-dev-stack.md` is the dev runbook; `005-auth.md` is the OIDC
  contract + CLI error-routing. Flow diagrams (upload / read /
  replace) live in `docs/architecture.md` -- consult when tracing
  how a request becomes slug-on-disk.
...
- `docs/zitadel.md` is operator-facing too: Zitadel project / role
  / app setup walkthrough, plus Zitadel-source citations (function
  names in `github.com/zitadel/zitadel` and
  `github.com/zitadel/oidc`) you may need when chasing auth bugs.
  Read it before touching anything OIDC-shaped.
- `docs/decisions/` holds ADRs for choices that must outlive a
  session. Filename convention: `YYYY-MM-DD-short-name.md`. Sections
  follow the pattern in `2026-05-17-device-code-vs-pkce.md`:
  Context / Constraint / Cost / Alternative / Decision / Triggers
  to reopen / Related.
